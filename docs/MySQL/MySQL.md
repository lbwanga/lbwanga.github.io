...
- **先执行子查询**，将结果集（如 `(1,2,3)`）物化到内存中
- 然后遍历主查询，逐行判断是否在子查询结果集中
...
- **先执行主查询**，每取一行数据，就执行子查询判断是否存在匹配记录
- 子查询一旦找到第一条匹配记录，立即返回 `TRUE` 并停止继续扫描（可短路）
