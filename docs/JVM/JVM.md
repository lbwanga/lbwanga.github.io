...
- 准备，为静态变量分配内存并设置默认值，赋值在初始化阶段，**这些内存都将在方法区中分配**。**实例变量会在对象实例化时随着对象一块分配在Java堆中**。final修饰的基本数据类型的静态变量，准备阶段直接会将代码中的值进行赋值。final修饰的引用类型的静态变量，会在初始化阶段赋值。
...
当类被 Java 虚拟机加载后， `.class` 文件中的**常量（类名、方法名、参数类型、字面量等）就存放在方法区的运行时常量池中**。
...
1. 初始标记：仅使用一条初始标记线程对所有与 GC Roots 直接关联的对象进行标记，速度很快，需要停顿。

2. 并发标记：使用多条标记线程，与用户线程并发执行。此过程进行可达性分析，标记出所有废弃对象。速度很慢。

3. 重新标记：使用多条标记线程并发执行，将刚才并发标记过程中新出现的废弃对象标记出来，需要停顿。
